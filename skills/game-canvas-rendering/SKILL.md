---
name: game-canvas-rendering
description: "Canvas Rendering & Effects — Action / visual-heavy games. Layered pipeline: procedural art, pooled particles, screenshake. Use when building or reviewing 2D browser games in plain HTML/JS/CSS."
...
> Canvas 2D craft: fixed draw order (bg→entities→fx→HUD), procedural shapes instead of image assets, particle pools with caps, screenshake and flash, no allocations inside the loop, offscreen caching for static layers.
...
| **LAYERS** | bg → entities → fx → HUD | Fixed order; HUD last |
| **ART** | Procedural shapes > images | Zero-asset, instant load |
| **FX** | Particle pool (capped) | Pooled objects, never new() in loop |
| **PERF** | Static layer offscreen cache | Redraw only dirty regions |
